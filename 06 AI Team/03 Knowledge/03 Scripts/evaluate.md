...
- Writes `05 Evaluations/<date>-<slug>.md` (result table, reviewer scorecard, gate table, findings, method) and updates the post's `yt2b_score` and `yt2b_status`. `reviewed` requires the complete rubric, including all six gates. Any failed condition produces `blocked`.
