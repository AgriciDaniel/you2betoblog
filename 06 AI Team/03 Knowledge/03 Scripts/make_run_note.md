...
**Outputs.** The note with the schema fields (`video_id, video_url, title, channel, channel_url, published, duration_s, rights, mode, status, captions, blogs, queue, approvals, created, updated, tags`) plus the media properties `thumbnail` (vault-relative path to `source/thumbnail.jpg`, empty when missing, usable as the image property of a Bases card view), `frames` (count of extracted frames) and `hero` (vault-relative path to the first blog's hero, only when it exists). `--add-blog` appends a wikilink to `blogs` once; `--log` appends `- <timestamp> <text>` and never duplicates a text. JSON: `run_note, status`.
